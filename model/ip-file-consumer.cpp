...
void
IPFileConsumer::PacketStatsUpdateEvent()
{
  m_currentStatsTrace(this, m_packetsSent, m_packetsReceived, m_packetsTimeout, m_packetsRetransmitted, EstimatedRTT, DeviationRTT);

  if (!m_active || m_finishedDownloadingFile == true)
    return;

  m_packetStatsUpdateEvent = Simulator::Schedule(Seconds(1), &IPFileConsumer::PacketStatsUpdateEvent, this);
}
...
void
IPFileConsumer::StartApplication() // Called at time specified by Start
{
  NS_LOG_FUNCTION_NOARGS();
...
void
IPFileConsumer::StopApplication() // Called at time specified by Stop
{
  NS_LOG_FUNCTION_NOARGS();
...
void
IPFileConsumer::AfterSendPacket(){
    m_packetsSent++;
}
...
bool
IPFileConsumer::SendManifestPacket(){
  if (!m_active)
    return false;
...
bool
IPFileConsumer::AreAllSeqReceived()
{
  uint32_t seqNo = 0;
...
void
IPFileConsumer::CreateTimeoutEvent(uint32_t seqNo, uint32_t timeout)
{
  ...
  m_chunkTimeoutEvents[seqNo] = Simulator::Schedule(MilliSeconds(timeout+1), &IPFileConsumer::CheckSeqForTimeout, this, seqNo);
}
...
void
IPFileConsumer::CheckSeqForTimeout(uint32_t seqNo)
{
...
void
IPFileConsumer::OnTimeout(uint32_t seq_nr)
{
...
void
IPFileConsumer::AfterData(bool manifest, bool timeout, uint32_t seq_nr)
{
...
long
IPFileConsumer::GetFaceBitrate(uint32_t faceId)
{
...
uint16_t
IPFileConsumer::GetFaceMTU(uint32_t faceId)
{
  Ptr<ns3::PointToPointNetDevice> nd1 = GetNode ()->GetDevice(faceId)->GetObject<ns3::PointToPointNetDevice>();
...
void
IPFileConsumer::OnManifest(long fileSize)
{
...
void
IPFileConsumer::OnFileData(uint32_t seq_nr, const uint8_t* data, unsigned length)
{
...
void
IPFileConsumer::ScheduleNextSendEvent(double miliseconds)
{
  ...
  m_sendEvent = Simulator::Schedule(NanoSeconds(miliseconds*1000000.0), &IPFileConsumer::SendPacket, this);
...
double
IPFileConsumer::CalculateDownloadSpeed()
{
...
void
IPFileConsumer::OnFileReceived(unsigned status, unsigned length)
{
...
bool
IPFileConsumer::DecompressFile ( std::string source, std::string filename )
{
